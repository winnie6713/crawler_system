...
import wuit.crawler.database.PhonePostCodeDB;
import wuit.crawler.database.PhonePostCodeInfo;

/**
 *
 * @author lxl
 */
public class PhonePostCodeManage {
    private PhonePostCodeDB db;    
    JPanel panel1;    
    private MainTable main;
    private SubTable subTable;   
    
    public PhonePostCodeManage(){
        db = new PhonePostCodeDB();
        initComponents();
    }
...
//                .addComponent(panelRight, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
  ...
  //                  .addComponent(panelRight, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    class MainTable{
        ...
        private PhonePostCodeDB db;
        private List<PhonePostCodeInfo> rs = new ArrayList<PhonePostCodeInfo>();
        
        public MainTable(PhonePostCodeDB db){
            this.db = db;
        ...
        public void refresh(){
            if(db == null)
                db = new PhonePostCodeDB();
            if(rs == null)
                rs = new ArrayList<PhonePostCodeInfo>();
            else
                rs.clear();
            db.SelectProvince(rs);
            display();
...
//            mainTable.selectAll ();//选择所有行
    ...
    class SubTable{
        ...
        private List<PhonePostCodeInfo> rs = new ArrayList<PhonePostCodeInfo>();
        private PhonePostCodeDB db;
        public SubTable(PhonePostCodeDB db){
            this.db = db;
        }
        ...
        public void display( ){
            ...
            for(int i=0; i<rs.size();i++){
                if(rs.get(i).stat == 3)
                    continue;
                String[] row = {rowNum+"",rs.get(i).code,rs.get(i).province,rs.get(i).city,rs.get(i).county,rs.get(i).phone};
        ...
        public void getSubInfo(String province){
            rs.clear();
            db.SelectPhonePostByProvince(province, rs);
        }
...
//                    db.InsertFeature(_feature);
                ...
                if(rs.get(i).stat == 2){
                    db.UpdatePhonePost(rs.get(i).id, rs.get(i).code, rs.get(i).province, rs.get(i).city, rs.get(i).county, rs.get(i).post, rs.get(i).phone);
                    rs.get(i).stat = 0;
                    continue;
                }
                if(rs.get(i).stat == 3){

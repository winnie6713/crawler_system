...
public class PageHtmlClear {
        ...
        public void save(){
            if(rowClear>=0)
                update();
            for(int i=0; i<rs.size(); i++){
                PageLocalInfo _clear = new PageLocalInfo();                
                _clear.name = rs.get(i).name;
                _clear.content = rs.get(i).content;
...
//                    db.InsertFeature(_feature);
                ...
                if(rs.get(i).state == 2){
                    db.UpdatePageClear(rs.get(i).content, rs.get(i).name, rs.get(i).id);;
                    rs.get(i).state = 0;
                    continue;
                }
                if(rs.get(i).state == 3){
